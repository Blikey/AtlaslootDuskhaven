...
function AtlasLoot_OnVariablesLoaded()
	...
	AtlasLoot.db.profile.MythicMode = false
end
...
function AtlasLoot_GetLoottableHeroic(dataID)
	local NormalID, HeroicID, Normal25ID, Heroic25ID, MythicID = nil,nil,nil,nil
	...
	dataID = gsub(dataID, "MYTHIC", "")			-- Mythic Table
	...
	if dataSource[dataID.."MYTHIC"] or dataSource[dataID.."MYTHIC".."_H"] or dataSource[dataID.."MYTHIC".."_A"] then
		MythicID = dataID.."MYTHIC"
		if englishFaction == "Horde" and dataSource[MythicID.."_H"] then
			MythicID = MythicID.."_H"
		elseif englishFaction ~= "Horde" and dataSource[MythicID.."_A"] then
			MythicID = MythicID.."_A"
		end
	end
	...
	return NormalID, HeroicID, Normal25ID, Heroic25ID, MythicID
end
...
function AtlasLoot_ShowItemsFrame(dataID, dataSource, boss, pFrame)
	...
	AtlasLootItemsFrame_Mythic:Hide();
	...
	--Set up checks to see if we have a heroic loot table or not
	local NormalID, HeroicID, Normal25ID, Heroic25ID, MythicID = AtlasLoot_GetLoottableHeroic(dataID)
	...
	elseif AtlasLoot.db.profile.BigraidHeroic and Heroic25ID then
		dataID = Heroic25ID
	elseif AtlasLoot.db.profile.MythicMode and MythicID then
		dataID = MythicID
	else
		if not NormalID then
			...
			elseif Heroic25ID then
				...
				AtlasLoot.db.profile.HeroicMode = false
				AtlasLoot.db.profile.MythicMode = false
			elseif MythicID then
				dataID = MythicID
				AtlasLoot.db.profile.Bigraid = false
				AtlasLoot.db.profile.BigraidHeroic = false
				AtlasLoot.db.profile.HeroicMode = false
				AtlasLoot.db.profile.MythicMode = true
			end
		else
			dataID = NormalID
		end
	...
	if (dataID == "SearchResult") or (dataID == "WishList") or (AtlasLoot_IsLootTableAvailable(dataID)) then
		...
		NormalID, HeroicID, Normal25ID, Heroic25ID, MythicID = AtlasLoot_GetLoottableHeroic(AtlasLootItemsFrame.refreshOri[1])
		...
		elseif NormalID and HeroicID then
			...
			AtlasLootItemsFrame_Heroic:Enable()
		elseif AtlasLoot.db.profile.MythicMode and MythicID then
			AtlasLootItemsFrame_Mythic:Show()
			AtlasLootItemsFrame_Mythic:SetChecked(true)
			AtlasLootItemsFrame_Mythic:Enable()
			if NormalID then
				AtlasLootItemsFrame_Mythic:Enable()
			else
				AtlasLootItemsFrame_Mythic:Disable()
			end
		elseif NormalID and MythicID then
			AtlasLootItemsFrame_Mythic:Show()
			AtlasLootItemsFrame_Mythic:SetChecked(false)
			AtlasLootItemsFrame_Mythic:Enable()
		end
...
function AtlasLoot_HeroicModeToggle()
	...
	elseif AtlasLoot.db.profile.BigraidHeroic and Normal25ID then
		...
		AtlasLoot.db.profile.MythicMode = false
		dataID = Normal25ID
	elseif AtlasLoot.db.profile.MythicMode then
		AtlasLoot.db.profile.MythicMode = false
		dataID = NormalID
	elseif MythicID then
		AtlasLoot.db.profile.Bigraid = false
...
function AtlasLoot_10Man25ManToggle()
	...
	local NormalID, HeroicID, Normal25ID, Heroic25ID, MythicID = AtlasLoot_GetLoottableHeroic(Lootpage)
